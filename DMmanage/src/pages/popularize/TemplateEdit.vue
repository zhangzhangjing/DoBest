		...
		<el-row>
			<el-col :span="20" class="toolbar" style="padding-bottom: 0px;">
				<el-form :inline="true">
					...
					<!-- <el-form-item>
						<el-button :loading="loading" @click="refreshBtnTapped">刷新作品</el-button>
					</el-form-item> -->
					<el-form-item>
			...
			<el-col :span="4" class="toolbar" style="padding-bottom: 0px;">
				<!-- <a :href="QRImageURL" download="yiyaqr.jpg">

				</a> -->
				<img :src="QRImageURL" class="QRimage">
				<el-button type="text" class="QR-text" :loading="downloading" @click="downloadQRImage">点击下载二维码</el-button>
				<!-- <p class="QR-text">点击下载二维码</p> -->
			</el-col>
					...
					<el-card :body-style="{ padding: '0px' }" style="" class="card">
				      ...
				      <div style="padding: 14px; bottom: 0px;">
				        <span><b>{{item.omitTitle}}</b></span>
				      	...
				      	<p style="max-height: 38px;overflow: hidden;margin-top: 0px;margin-bottom: 0px;">{{item.omitContent}}</p>
			...
			<el-form :model="addForm" label-width="120px" :rules="addFormRules" ref="addForm">
				...
				<template v-else>
					<el-form-item label="标题" prop="title">
						<el-input v-model="addForm.title" :maxlength="15" auto-complete="off"></el-input>
					</el-form-item>
		...
		methods: {
			...
			workDetailFetch: function () {
				...
				yiya.workDetail4EditFetch(workID).then(response =>{
					...
					// if(photos.length == 0 && that.workInitTimes <= 3)
					// {
					// 	that.workInitTimes += 1;
					// 	yiya.workInit(workID).then(response =>{
					// 		that.workDetailFetch();
					// 	})
					// }
			...
			templateQRImageURLFetch:function(){
				var workID = this.workID;
				yiya.templateQRImageURLFetch(workID).then(response =>{
			...
			downloadQRImage:function(){
				this.downloading = true;
				yiya.templateQRImageDownloadURLFetch(this.workID).then(relativePath =>{
			...
			addBtnTapped: function () {
				...
				// for (var i = 0; i < photos.length; i++) {
				// 	var photo = photos[i];
				// 	if(!photo.imgurl && photo.pdid)
				// 	{
				// 		// this.addFormVisible = true;
				// 		// var form = {pid:photo.pdid, title: '', content:'', photoURL:'', photoData:''};
				// 		// this.addForm = form;
				// 		this.editItem(photo);
				// 		return;
				// 	}
			...
			editItem:function(item)
			{
				...
				this.isCover = item.sort == 0 || this.photos.length == 0;

				this.addFormVisible = true;
				var form = {pid:item.pdid, title:item.title, content:item.content, description:item.description, photoURL:item.imgurl, photoData:''};
				this.addForm = form;
			},
			...
			pageModify:function(){
				...
				var description = StringUtil.trim(this.addForm.description);
				...
				// yiya.uploadBase64Image(data, parseInt(canvasWidth), parseInt(canvasHeight)).then(response =>{
				// 		//return yiya.xxx
				// 	}).catch(error =>{
