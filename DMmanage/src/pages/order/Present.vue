<template>
	<section>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form>
				<el-form-item>
					<span><b>功能含义</b>：指员工使用自己新建的作品，一对一邀请潜在客户</br></span>
					<span><b>板块说明</b>：员工成功邀请的客户，会留下手机号码地址等信息（利用此功能，可灵活开展市场活动）</span>
				</el-form-item>
			</el-form>
		</el-col>
		...
		<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
			<el-form :inline="true">
				...
				<el-form-item>
					<!-- <el-button type="primary" v-on:click="createBtnClicked">创建作品</el-button> -->
					<el-dropdown split-button type="primary" @click="createBtnClicked">创建作品
						<el-dropdown-menu slot="dropdown">
							<el-dropdown-item @click.native="viewWorks">我的作品</el-dropdown-item>
						</el-dropdown-menu>
					</el-dropdown>
				</el-form-item>
			</el-form>
		...
		<el-dialog title="创建作品" v-model="workFormVisible" :close-on-click-modal="false">
			...
			<div slot="footer" class="dialog-footer">
				<el-button @click.native="workFormVisible = false">取消</el-button>
				<el-button type="primary" @click.native="createWork" :loading="addLoading">提交</el-button>
			</div>
		</el-dialog>
	...
	export default {
		data() {
			return {
				...
				workFormVisible: false,//新增界面是否显示
				addLoading: false,
				addFormRules: {
					name: [
		...
		methods: {
				...
				yiya.presentRecordsFetch(this.category, this.page, this.status, this.keyword).then(response => {
					...
					for(var i = 0;i < list.length; i++)
					{
						...
						if(item.inviteModel)
						{
							...
							item.inviteeUID = item.inviteModel.userid;
						}
			...
			productsFetch:function(){
				yiya.productsFetch().then((response) => {
					...
					if(firstItem )
					{
						...
						form.productID = firstItem.value;
						this.addForm = form;
					}
				})
			},
			...
			viewWorks:function()
			{
				this.$router.push({ name: 'MineWorks'});
			},
			...
			createBtnClicked:function(){
				...
				this.addForm = {productID:'', title:'', author:''};
			...
			createWork(){
				var form = this.addForm;
				var info = {productid:form.productID, title:form.title, author:form.author};
				this.addLoading = true;

				yiya.createWork(info).then(response =>{
					console.log(response);
					this.workFormVisible = false;
					this.addLoading = false;
					this.$notify({ title: '成功', message: '创建成功', type: 'success' });
					var workID = response.cartId;
					if (workID)
					{
					...
					this.addLoading = false;
					this.$notify({ title: '错误', message: error.message || "创建失败", type: 'error' });
				});
			}
