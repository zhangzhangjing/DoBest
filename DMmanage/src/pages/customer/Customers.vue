	...
	<section>
		...
		<el-row>
			<el-col :span="24" class="toolbar">
				<el-form>
					<el-form-item>
						<span><b>功能含义</b>：这里显示的是本店通过各种各样活动获取的所有客户的列表，客户的详情信息。</br></span>
						<span><b>板块说明</b>：可以通过客户的昵称、手机号来搜索客户。</span>
					</el-form-item>
				</el-form>
			</el-col>
		</el-row>
		...
		<el-row>
			<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
				<el-form :inline="true">
					...
					<el-form-item>
						<el-select v-model="sourceType" placeholder="请选择" @change="sourceTypeChanged">
	    					<el-option label="全部" value=""></el-option>
	    					<el-option label="普通活动" value="2"></el-option>
	    					<el-option label="影楼一对一" value="1"></el-option>
	    					<el-option label="客户一对一" value="5"></el-option>
	    					<el-option label="其它" value="4"></el-option>
	    					<!-- <el-option label="订单分配" value="3"></el-option> -->
	  					</el-select>
					</el-form-item>
					<el-form-item>
		...
		<el-table :data="users" highlight-current-row v-loading="listLoading" style="width: 100%;">
			...
			<el-table-column prop="name" label="客户昵称" min-width="160" sortable :sort-method="nameSort"></el-table-column>
			...
			<el-table-column prop="createtimeStr" label="注册时间" width="180" sortable></el-table-column>
				...
				<template scope="scope">
					<el-popover trigger="click" placement="top" class="popover">
						<div style="max-height:300px; overflow:auto;">
							<el-button type="text" class="order-button" v-for="item in scope.row.cartIdList" @click="viewWork(item.cartid)">{{item.title || '作品'}}</el-button>
						</div>
			          	<el-button slot="reference" size="small" :disabled="scope.row.worksCount == 0">查看作品{{scope.row.worksCount}}个</el-button>
			        </el-popover>
	...
	export default {
		...
		methods: {
			...
			getUsers:function(){
				...
				NProgress.start();
				yiya.customersListFetch(this.page, this.keyword, this.startDateString, this.endDateString, this.sourceType).then((response) => {
