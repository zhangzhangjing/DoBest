  ...
  export default {
    ...
    methods: {
      handleSubmit2(ev) {
            ...
            yiya.login(phone, password).then(response => {
              ...
              {
                    RouteUtil.addRoutes(this.$router, freelanceRoutes);
              }
              else
              {
                    // RouteUtil.addRoutes(this.$router, generalRoutes);
                    RouteUtil.addRoutes(this.$router, RouteUtil.routes);
              ...
              if (Role.permitted(identity,Role.Printer))
              {
                    that.$router.push({ path: '/order_factory' });
              }
              else if (Role.permitted(identity,Role.CTSAdmin | Role.CTSUser))
              {
                    that.$router.push({ path: '/feedbacks' });
              }
              else if (Role.permitted(identity,Role.Agent | Role.Branch))
              {
                    that.$router.push({ path: '/overview' });
              }
              else if (Role.permitted(identity,Role.Freelance))
              {
                    that.$router.push({ path: '/trafficsources' });
              }
              else
              {
      ...
      {
            var addedRoutes = [];
            var localRoutes = this.$router.options.routes;
            ...
            this.$router.options.routes = localRoutes.concat(addedRoutes);
      }
