<template>
	<el-row class="container">
		...
		<el-col :span="24" class="main">
			<aside>
				<el-menu :default-active="$route.path" class="el-menu-vertical" @open="handleopen" @close="handleclose" @select="handleselect" background-color="#324157"
								 text-color="#bfcbd9"	theme="dark" unique-opened router>
	...
	export default {
		...
		methods: {
			...
			close:function(){
				...
				if(window.location.href.indexOf("review/dealer") !== -1){
      				this.$router.push('/login');
				}else if(window.location.href.indexOf("partner/trafficsources") !== -1){
      				this.$router.push('/login');
				}else {
          			window.history.go(-1);
				}
			...
			// this.Role.permitted(identity,item.privileges || 0)
		},
		mounted() {
			...
			this.$router.options.routes.forEach(item => {
				if ((!item.hidden) && Role.permitted(this.identity,item.privileges || 0)) {
					if (this.RoleIndex !== item.privileges) {
						this.RoleIndex = item.privileges
						switch(this.RoleIndex)
							{
								case 2:
							    	...
							    	break;
							  	case 1024:
							    	this.RoleName= "DM管理后台：商户";
							    	break;
							    case 256:
							    	this.RoleName = "DM管理后台：代理商";
							    	break;
							    case 192:
							    	this.RoleName = "CTS管理后台";
							    	break;
							    case 2048:
							    	this.RoleName = "CTS管理后台";
							    	break;
							    case 512:
							      this.RoleName = "DM管理后台：生产商";
							      break;
							}
					}
				}		
