	...
	<section>
			...
			<el-col :span="24" class="toolbar">
				<el-form>
					<el-form-item>
						...
						<span><b>如何使用：</b>新增好1个DM活动，今后只需要点击“业务链接”扫码（或在手机企业工作台）进入活动首页，把活动首页发送给客户即可（每次发送只能选择一个客户）。</span>						
					</el-form-item>
				</el-form>
			</el-col>
		</el-row>
				...
				<el-form>
					<el-form-item>
						<div class="workbenchimgBox">
							...
							<p style="padding:0px;margin:0px;margin-top: -10px;">电脑发活动邀请太麻烦？</p>
							<p style="padding:0px;margin:0px;margin-top: -10px;">扫码即可手机发送邀请</p>
						</div>
					</el-form-item>
				</el-form>
			</el-col>
		...
		<el-row>
			<el-col :span="24" class="toolbar" style="padding-bottom: 0px;">
				<el-form :inline="true">
					<el-form-item>
						<el-button type="primary" @click="handleAdd">新增1对1DM</el-button>
					</el-form-item>
				</el-form>
			</el-col>
		</el-row>
		...
		<el-table :data="templates" highlight-current-row v-loading="listLoading" style="width: 100%;">
			...
			<!-- v-if="type == 0 || type == 2" -->
			<el-table-column  prop="remark" label="活动链接" width="100">
				<template scope="scope">
		...
		<!--新增界面-->
		<el-dialog title="创建活动" :visible.sync="addFormVisible" :close-on-click-modal="false">
			<template>
				<el-form :model="addForm" label-width="120px" label-position="left" :rules="addFormRules" ref="addForm0">
	                ...
	                <el-form-item label="二次传播">
					    <el-radio v-model="addForm.zanType" :label="0">
					    	<span style="font-size:11px;">（点赞人赠送优惠券）</span>
					    </el-radio>
					</el-form-item>
				...
				<el-form :model="addForm" label-width="140px" label-position="left" :rules="addFormRules" ref="addForm1">
					<el-form-item label="赠送方式" >
					    <el-radio-group v-model="addForm.presentOption" >
							<el-radio :label="1" >一对一发送模式（每次仅1人可领取，非特殊需求不要使用）</el-radio>
					    </el-radio-group>
					</el-form-item>
					<!-- <el-form-item label="参与限制"  >
					    <el-radio-group v-model="addForm.limitOption">
					      	<el-radio :label="1" :disabled="addForm.presentOption ==1">
					      		<el-input v-model="addForm.limitHumanCount" :disabled="addForm.limitOption != 1" style="width: 60px;"></el-input>
					      		<span>人能获得参与资格</span>
					      	</el-radio>
						    <el-radio :disabled="addForm.presentOption == 2" :label="0">无限制（一般不修改）</el-radio>
					    </el-radio-group>
					</el-form-item> -->
<!-- 					<el-form-item  label-width="190px" label="活动礼品只能寄往本店地址">
			...
			<div slot="footer" class="dialog-footer">
				<el-button  type="text" @click.native="addFormVisible = false">取消</el-button>
				<el-button type="primary" @click.native="addSubmitBtnClicked">确定</el-button>
			</div>
		</el-dialog>
...
<!-- 				<span style="color: red; margin-top: -25px; float: left;">
					(修改此项会导致部分达标作品无法自动下单，请手动为其下单)
				</span> -->
			</el-form>
	...
	export default {
		...
		methods: {
			...
			campaignsFetch:function(){
				...
				api.ibs.campaignsFetch(this.page, this.type, this.keyword).then((response) => {
					...
					for(var i = 0;i < list.length; i++)
					{
						...
						item.limitHumanCount = item.freecount;
						item.workID = item.cartid;
						// item.downloading = false; keletest生成广告图
					}
					that.templates = list;
			...
			handleAdd: function () {
				...
				// limitOption:0,hoursEffective:hoursMinEffective,
				var form = {name: name, productID:'', styleID:'',guidelines:guidelines, presentOption:this.type,shareCountX:1,shareCountX2:0, limitHumanCount:0, applylimitcount:shareMinCount2,shareCount:shareMinCount};
			...
			addSubmit: function () {
				...
				// 	};
				// 	// 限时
				// 	if (this.addForm.hoursEffective < 1 || this.addForm.hoursEffective> 96) {
				// 		this.$notify({ title: '错误', message:"名额作废请输入1~96的数字!", type: 'error' });
				// 		return;
				// 	} else {
				// 		params.hoursEffective = this.addForm.hoursEffective;
				// 	}
				// } else {
